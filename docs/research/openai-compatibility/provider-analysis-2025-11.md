...
2. **Azure OpenAI** - Enterprise with Microsoft infrastructure
3. **Google Vertex AI** - Multi-model including OpenAI-compatible endpoints
4. **AWS Bedrock** - Multi-model marketplace

#### High-Speed/Low-Cost Inference
5. **Groq** - Fastest inference (LPU architecture), native OpenAI compatibility
   - Base URL: `https://api.groq.com/openai/v1`
   - Auth: `Authorization: Bearer <api_key>`
6. **Together AI** - Wide model selection, cost-effective
7. **Fireworks AI** - Fast inference, competitive pricing
8. **Anyscale Endpoints** - Serverless LLM hosting
9. **Cerebras** - Ultra-fast inference with custom hardware

#### Aggregators/Routers
10. **OpenRouter** - 300+ models unified API
    - Requires `HTTP-Referer` header for attribution
11. **Perplexity** - Search-augmented models
12. **LiteLLM** - Python library for 100+ providers (not a service)

#### Local/Self-Hosted
13. **Ollama** - Current implementation ✅
14. **LM Studio** - Desktop GUI with OpenAI API
15. **vLLM** - Open-source inference server
16. **LocalAI** - Self-hosted OpenAI alternative
...
2. Make `base_url`, `api_key`, `model` configurable (remove defaults)
3. Add factory methods:
   ...
   ```
4. Move GCP auth to separate strategy/mixin
5. Add `organization`, `timeout`, `max_retries` parameters
6. Support `stream_options` for usage metadata in streaming
...
2. **Capability detection** - Query `/models` endpoint, feature flags
3. **Error normalization** - Standard error types across providers
4. **Retry/fallback** - Automatic provider switching on failure
5. **Multimodal support** - Images, audio (extends beyond current text-only)
6. **Observability** - Provider-specific metrics, cost tracking
7. **Model routing** - Cost/latency-based provider selection
...
2. **RFC drafting**: Once direction chosen, draft formal RFC with:
...
3. **Prototype**: Build quick proof-of-concept with OpenAI + Groq support
